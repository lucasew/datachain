...
class Database:
        ...
        def checker(env, item):
            ...
            if "validation_type" in param:
                assert truep(
                    ...
                    _eval(
                        {**env, "item": item},
                        [f'validate_{param["validation_type"]}', ["var", "item"]],
                    ),
                )
            if "check" in param:
                assert item is not None
                assert truep(env, _eval({**env, "item": item}, param["check"]))
            return True
    ...
    def _setup(self):
        ...
        base_env = dict(db=self)

        for type_name, type_def in header["types"].items():
            base_env[f"validate_{type_name}"] = self._get_checker(type_def)

        for op_name, op in header["ops"].items():

            def make_op_payload(op_def):
                def op_payload(env):
                    handled_args = dict()
                    for param_name, param in op_def["params"].items():
                    ...
                    return env["eval"](env, op_def["body"])

                return op_payload

            register = evaluator_item(name=op_name, register=False)

            base_env[op_name] = register(make_op_payload(op))
