...
class Database:
    def __init__(self, chainfile):
        ...
        with self.chainfile.open("r") as f:
            next(f)  # skip header
            for body_item in f:
                self._handle_body_item(body_item)
                # report_error('result', result=result)
    ...
    def _handle_body_item(self, item):
        # report_error('body_item', item=item)
    ...
    def _setup(self):
        ...
        if "allowed_keys" in header:
            ...
            # report_error('allowed_keys', allowed_keys=header['allowed_keys'])
        ...
        for op_name, op in header["ops"].items():
            ...
            def op_payload(env):
                ...
                for param_name, param in op["params"].items():
                    ...
                    # report_error('param', param_name=param_name, item=item)
    ...
    def sql(self, query, *args):
        # report_error('sql', query=query, args=args)
