        ...
        with (this_dir / "example.body.json").open("r") as f:
            for line in f:
                try:
                    ...
                    # Expected if line is part of header or non-json? Actually header is handled
                    # Or it's for invalid json. We should probably report it if it's considered an error,
                    # but test doesn't actually check output, just concats. We'll just report it
                    report_error("Failed to decode JSON", error=e, line=line)
                    print(line, file=out)
    handle_test_with_concatenated_file(
        signed_concatenated_file, is_the_signed_test=True
    )
    ...
    with concatenated_file.open("r") as f:
        for i, line in enumerate(f):
            ...
            try:
                ...
                report_error(
                    "Exception while parsing line", error=e, line_no=i + 1, line=line
                )
                raise e

    db = Database(concatenated_file)
    # report_error('header', header=db._header)
    # report_error('db_id', db_id=db.db_id)

    # report_error('idx result', result=db.sql('select * from idx'))
...
def test_evaluator_basic():
    e = Evaluator(dict(a=2, b=2))
    # print(e.env.keys())
    # print(e.env.get('var'))
    # report_error('eval result', result=e.eval(['+', ['var', 'a'], ['var', 'b']]))
...
def test_evaluator_lazy():
    ...
    e = Evaluator(dict(cond=True, trigger_call=trigger.trigger_from_eval))
